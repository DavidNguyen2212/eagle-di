...
if __name__ == "__main__":
    # This section demonstrates usage patterns
    ...
    # Assume Injectable is available from DI framework
    # @Injectable
    class UserRepository:
        ...
        @transactional
        async def create_user(self, name: str, db=None) -> User:
        ...
        @transactional(propagation=Propagation.REQUIRES_NEW)
        async def create_audit_log(self, action: str, db=None):
            """Always create new transaction (commits even if parent rolls back)."""
            # Audit log persists even if parent transaction fails
            pass
        ...
        @transactional(
            ...
            no_rollback_for=(UserWarning,)
        )
        async def try_update_user(self, user_id: int, name: str, db=None):
            """Try to update, rollback only this operation on ValueError."""
            if not name:
    ...
    async def example_fastapi_endpoint():
        db_provider = DatabaseProvider("postgresql+asyncpg://user:pass@localhost/db")
        
        # Method 1: Service handles transaction automatically
        user_repo = UserRepository(db_provider)
        user = await user_repo.create_user("John")  # Auto-commit
